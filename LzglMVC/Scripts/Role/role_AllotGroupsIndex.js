$(function(){
    ...
    $(".opt-groups").click(function () {
        //添加权限组名称，隐藏域
        $("#selected-groups").attr("value", $(this).val());
        //该角色下的所有权限组列表
        //$.getJSON($("#rootpath").val() + "/Groups/GetGroupsList", { RoleId: $(this).val() }, function (data) {
        //    $('#show-role option').remove();
        //    for (var i = 0; i < data.length; i++) {
        //        var opt = '<option title="' + data[i]['Note'] + '" values="' + data[i]['Gid'] +
        //            '">' + data[i]['Title'] + '</option>';
        //        $('#show-role').append(opt);
        //    }
        //});
    });
    //给角色分配权限
    $('#GroupsToRole').click(function(){
        var rid = $("#selected-role").val();
        var gid = $("#selected-groups").val();
        $.getJSON(
            $("#rootpath").val() + "/Role/AllotGroups",
            {
                rid: $("#selected-role").val(),
                gid: $("#selected-groups").val()
            },
            function (data) {
                $('#show-groups option').remove();
                $('#show-groups').attr('size', data.length);
    ...
    $(".opt-groups").click(function () {
        //添加角色名称
        $("#selected-group").html($(this).html()).attr("value", $(this).val());
        //该角色下的所有权限组列表
        ...
        $.confirm({
            ...
            buttons: {
                确定: function () {
                    ...
                    //在移去权限组之后，刷新该角色限的所有权限组
                    $.getJSON($("#rootpath").val() + "/Actions/GetActionsList", { GroupsId: $("#selected-groups").val() }, function (data) {
                        $('#show-actions option').remove();
                        $('#show-actions').attr('size', data.length);
                        for (var i = 0; i < data.length; i++) {
                            var opt = '<option class="list-group-item"' + '" value="' + data[i]['Actid'] +
